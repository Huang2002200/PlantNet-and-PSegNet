...
def discriminative_loss_single(prediction, correct_label, feature_dim,
    ...
    # prediction:网络推理得到的实例特征向量
    reshaped_pred = tf.reshape(prediction, [-1, feature_dim])
    # Count instances，unique_labels：实例标签组成的tensor
    # unique_id：实例标签对应的索引，counts：Nt,每个实例类的点数
    unique_labels, unique_id, counts = tf.unique_with_counts(correct_label)
    counts = tf.cast(counts, tf.float32)
    # num_instances：|I|点云总实例数
    num_instances = tf.size(unique_labels)
    #tf.unsorted_segment_sum分段求和，同一实例类的点的特征向量加起来
    segmented_sum = tf.unsorted_segment_sum(reshaped_pred, unique_id, num_instances)
    # mu: μt实例的聚类中心,同一实例的所有特征向量的重心
    mu = tf.div(segmented_sum, tf.reshape(counts, (-1, 1)))
    mu_expand = tf.gather(mu, unique_id)#用unique_id将mu对应扩充为4096个点
    # Calculate l_var: 𝐿_𝑠𝑎𝑚𝑒
    tmp_distance = reshaped_pred - mu_expand  #μt-ei
