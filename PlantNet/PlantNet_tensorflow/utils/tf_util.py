...
def get_edge_feature(point_cloud, nn_idx, k=20):
  ...
  idx_ = tf.range(batch_size) * num_points#(16,)
  idx_ = tf.reshape(idx_, [batch_size, 1, 1]) #(16,1,1)

  point_cloud_flat = tf.reshape(point_cloud, [-1, num_dims])#point_cloud_flat（16384,6）
  point_cloud_neighbors = tf.gather(point_cloud_flat, nn_idx+idx_)#抽取出params的第axis维度上在indices里面所有的index,nn_idx+idx_(16,1096,16)
  point_cloud_central = tf.expand_dims(point_cloud_central, axis=-2)#(16,1096,1,6)

  point_cloud_central = tf.tile(point_cloud_central, [1, 1, k, 1])#tf.tile（）复制
