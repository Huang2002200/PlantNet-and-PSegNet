...
parser.add_argument('--input_list_train', type=str, default='PlantNet/data/train_file_list.txt', help='Input data list file')
parser.add_argument('--input_list_test', type=str, default='PlantNet/data/test_file_list.txt', help='Input data list file')
parser.add_argument('--restore_model', type=str, default='', help='Pretrained model')
...
LOG_DIR="/media/david/HDD1/zzy/h5File/FPS_2048_log"  #the saved path of log
...
for h5_filename in train_file_list:
    ...
    train_group.append(cur_group)  # ins label
    train_sem.append(cur_sem)   # sem label
    train_obj.append(cur_obj)  # object label
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            loss, sem_loss, disc_loss, l_var, l_dist, l_reg, simmat_loss = \
                get_loss(pred_ins, labels_pl, pred_sem, sem_labels_pl, fuse_catch, ptsseglabel_ph,
                         ptsgroup_label_ph, pts_seglabel_mask_ph, pts_group_mask_ph)
        ...
        for epoch in range(MAX_EPOCH):
            ...
            best_model_signal, best_test_accuracy=eval_one_epoch(sess, ops, test_writer,best_test_accuracy)
            # Save the variables to disk.
            if best_model_signal:
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        (summary, step, _, loss_val, sem_loss_val, disc_loss_val, l_var_val, l_dist_val, l_reg_val,l_simmat_loss, pred_val) = sess.run([ops['merged'],
                            ops['step'], ops['train_op'], ops['loss'], ops['sem_loss'], ops['disc_loss'], ops['l_var'], ops['l_dist'], ops['l_reg'],
                                                                            ops['simmat_loss'], ops['pred_sem']], feed_dict=feed_dict)
...
def eval_one_epoch(sess, ops, test_writer,best_test_accuracy):
    """ ops: dict mapping from string to tf ops """
    is_training = False
    total_correct = 0
    total_seen = 0
    ...
    for batch_idx in range(num_batches+1):
        ...
        feed_dict = {ops['pointclouds_pl']: current_data[start_idx:end_idx, :, :],
                     ...
                     ops['pts_group_mask_ph']: pts_group_mask,
                     ops['is_training_pl']: is_training,}
        summary, step, loss_val, pred_val_all = sess.run([ops['merged'],
                                                ops['step'], ops['loss'], ops['pred_sem']],
                                                feed_dict=feed_dict)
    ...
    best_model_signal=False
    print("best_test_accuracyian",best_test_accuracy)
    if test_acc > best_test_accuracy:
        best_test_accuracy=test_acc
        print("best_test_accuracy：",best_test_accuracy)
        best_model_signal = True
    return best_model_signal, best_test_accuracy
