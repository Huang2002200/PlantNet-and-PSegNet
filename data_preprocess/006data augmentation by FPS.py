...
class FarthestSampler:
    ...
    def _calc_distances(self, p0, points):
        return ((p0 - points) ** 2).sum(
            axis=1)  # Returns the sum of squared Euclidean distances between the set of sample points and other points

    def _call__(self, pts, k):  # PTS is the input point cloud,  K is the number of downsampling
        farthest_pts = np.zeros((k, 5),
...
if __name__ == '__main__':
    ...
    for idx in range(n):
        ...
        for z in range(10):  # do 10 times data augmentation
            ...
            np.savetxt(os.path.join(saved_path, file_nameR), sample_points, fmt='%f %f %f %f %d')
